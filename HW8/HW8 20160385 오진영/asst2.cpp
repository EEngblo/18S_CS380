...
static Mesh messiMesh; // 메인 mesh의 이름은 메시임. Korean joosuk makes error?
...
static vector<VertexPN> meshFormTriangle(Mesh &mesh){
...
static void subdivision(Mesh &mesh){
  ...
  for(int i = 0; i < nf; i++){
    ...
    for(int j = 0; j < f.getNumVertices(); j++)
      temp += f.getVertex(j).getPosition();

    temp /= f.getNumVertices();
    mesh.setNewFaceVertex(f, temp);
  }
  ...
  for(int i = 0; i < nv; i++){
    ...
    Mesh::VertexIterator it(v.getIterator()), it0(it);
    do{
      temp += it.getVertex().getPosition() + mesh.getNewFaceVertex(it.getFace());
      cnt++;
    }while (++it != it0);

    temp /= cnt;
    temp += v.getPosition() * (cnt - 2);
    temp /= cnt;

    mesh.setNewVertexVertex(v, temp);
  }
...
static void animateMeshTimerCallback(int ms) {
  ...
  for (int i = 0; i < g_subdivisionLevel; i++)
    subdivision(messiMesh);

  meshInitNormal(messiMesh);
  vector<VertexPN> vertices = meshFormTriangle(messiMesh);
...
static void initHW7(){
  // HW7을 위한 subdivision init임 : mesh 불러오기 등 수행
  defaultMesh = Mesh();
  defaultMesh.load("cube.mesh");

  meshInitNormal(defaultMesh);

  messiMesh = Mesh(defaultMesh);
  vector<VertexPN> vertices = meshFormTriangle(messiMesh);

  messi.reset(new SimpleGeometryPN());
  messi->upload(&vertices[0], vertices.size());
...
static void initMaterials() {
  ...
  // Specify a color for that material using the uColor uniform variable, and use it for drawing the subdivision mesh.
  g_specularMat.reset(new Material(specular));
  g_specularMat->getUniforms().put("uColor", Cvec3f(0.7f, 0.5f, 0.85f)); // 아마도 괴생물체의 색인듯?
...
static void initScene() {
  ...
  g_meshNode->addChild(shared_ptr<MyShapeNode>(
    new MyShapeNode(messi, g_specularMat, Cvec3(0,0,0))
  ));
